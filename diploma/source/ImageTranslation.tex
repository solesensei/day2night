...
\section{Постановка задачи}
	...
	\section{Обзор существующих методов}
		...
		Для этого введем функцию потерь $\mathcal{L}_{ae}$, характеризующую потери при неправильном принятии решений:
		\begin{equation}
		...
		Эту проблему способны решить некоторые дополнительные предположения, накладываемые на модель, а именно вариацонные приближения.
		...
		Кодировщик на вход получает пример $x$ и выдает вектор размерности $2d$ распределения скрытых переменных $z$, состоящий двух векторов размерности $d$:
		...
		На рисунке \ref{vaemodel} представлена верхнеуровневая схема модели вариационных автоэнкодеров.
		...
		Будем рассматривать уже введенные в формальном определении домена изображений \(X_{1}\) и \(X_{2}\), соответсвующие разному времени суток (глава \ref{intro_problem}).
