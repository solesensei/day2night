...
\section{Обзор существующих методов}
	...
	\subsection{Генеративно состязательные сети (GAN)}		
		...
		\end{equation}
		\begin{equation*}
			\mathcal{L}_{GAN}(D,G) = \mathbb{E}_{x∼p_{data}(x)}\big[\log{D(x)}\big] + \mathbb{E}_{x∼p_{z}(z)}\big[\log (1 - D(G(z)))\big]
		\end{equation*}
		\indent
	...
	\subsection{Методы использующие обучение с учителем}
		...
		\subsubsection{Условные состязательные сети (cGAN)} \label{sec:pix2pix}
			...
			\begin{equation}\label{eq:cgan}
				\arg\min_{G}\max_{D} \Big( \mathcal{L}_{cGAN}(D,G) + \lambda \mathcal{L}_1(G) \Big), \text{ где }
			\end{equation}
			\begin{equation*}
					\mathcal{L}_{cGAN}(D,G) = \mathbb{E}_{x}\big[\log{D(x)}\big] + \mathbb{E}_{y,z}\big[\log (1 - D(G(z,y)))\big],\;
	...
	\subsection{Методы использующие обучение без учителя}
		...
		\subsubsection{Цикловая согласованность (CC)}
			...
			тогда если изображение \(x \in X\) отоброжается в \(G(x) = y \in Y\), то изображение \(y\) отображается обратно в \(F(y) = x\).
			Ниже на рис. \ref{fig:cyc} изображена схема цикловой согласованности:
			...
			\begin{equation}\label{eq:cc}
				\mathcal{L}_{cc}(G,F) = \mathbb{E}_{x}\big[\| F(G(x))-x \|\big] + \mathbb{E}_{y}\big[\| G(F(y))-y \|\big]
			\end{equation}
		...
		\paragraph{Состязательные сети с цикловой согласованностью}
			Исследовательская группа при университете Беркли, предложившая метод переноса с учителем pix2pix \cite{pix2pix} (глава \ref{sec:pix2pix}), признала, что для многих задач переноса изображений парное обучение будет неприменимым.
			...
			Новый метод, получивший название CycleGAN, обрел популярность, появилось множество его модификаций и реализаций в том числе и с использованием PyTorch.
			\newline
			\newline
			...
			\begin{figure}[ht]
				...
				\caption{Схема работы модели переноса без учителя с цикловой согласованностью CycleGAN}{
					\small{
			...
			\end{itemize}
			\(D_X\) заставляет генератор \(G\) переносить изображения \(X\) в изображения неотличимые от домена \(Y\), аналогично \(D_Y\) поощряет \(F\) синтезировать изображения близкие к \(X\).
			Авторы ввели функции потерь цикловой согласованности \(\mathcal{L}_{cc}\) \eqref{eq:cc} и две функции состязательных потерь \(\mathcal{L}_{GAN_X}\) и \(\mathcal{L}_{GAN_Y}\) \eqref{eq:gan}.
			Получилась следующая задача оптимизации:

			\begin{equation}\label{eq:cyclegan}
				\arg\min_{G, F}\max_{D_X, D_Y}\mathcal{L}_{GAN_X}(D_X,F,Y,X) + \mathcal{L}_{GAN_Y}(D_Y,G,X,Y) + \lambda\mathcal{L}_{cc}(G,F)
			\end{equation}
			Верхнеуровневую схему работы алгоритма, можно увидеть на рис. \ref{fig:cyclegan}, более развернутую на рисунке \ref{fig:cyclegan_model}:

			\begin{figure}[ht]
				\centering
				\includegraphics[width=0.9\textwidth]{img/cyclegan_model}
				\caption[Схема работы модели CycleGAN (день в ночь)]{Схема работы модели CycleGAN (день в ночь)\footnotemark}
				\label{fig:cyclegan_model}
			\end{figure}  \footnotetext{ для случая ночь в день, аналогично}
